...
This example application shows how to use the CAN module to transmit and receive CAN FD<br /> messages in polling mode.
...
Path of the application within the repository is<br /> **apps/can/can\_fd\_operation\_blocking/firmware**.
...
8.  If the steps are executed in this sequence, the final output in the consoles will be as below \(console on the left is the transmitter \(board 1\) and the one on the right is receiver \(board 2\)\):
    ...
    ![](GUID-61B6994B-90DE-4CDA-9787-0B7CB35089CE-low.png)
